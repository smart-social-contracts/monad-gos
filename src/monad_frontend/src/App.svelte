<script lang="ts">
	...
	async function loadCore() {
		...
		try {
			...
			const message = e instanceof Error ? e.message : 'Failed to load Monad GOS';
			error = message;
	...
	async function bootstrap() {
		...
		if (isEmbeddedInPortal()) {
			...
			configureMonadGosRuntime({ canisterId: config.backendCanisterId });
		}
...
<div class="monad-gos-app">
	{#if setupActive}
		<div class="setup-shell">
			{#if loading}
				<p class="monad-gos-muted">Loading setup…</p>
			{:else if needsAuth}
	...
	{:else}
		<div class="chat-shell">
			<div class="chat-column">
				<div class="floating-chrome" aria-label="Monad GOS navigation">
					<AppHeader
					...
					<nav class="nav" aria-label="Monad GOS sections">
						<button
							type="button"
							class="nav-btn"
						...
						{#if isMockMode() && !isEmbeddedInPortal()}
							<span class="mock-badge monad-gos-muted">Mock</span>
						{/if}
					</nav>
				...
				<main class="chat-scroll">
					<div class="chat-content">
						{#if loading}
							<p class="monad-gos-muted">Loading Monad GOS…</p>
						{:else if error && connectionStatus === 'error'}
							<p class="error">{error}</p>
							<button type="button" class="monad-gos-btn" onclick={bootstrap}>Retry</button>
					...
					<div class="composer-dock">
						{#if view === 'broadcast' && replyBroadcastId}
							<div class="composer-meta">
								<span class="monad-gos-muted">Replying · opens a private thread</span>
...
<style>
	...
	.floating-chrome {
		...
		border: 1px solid color-mix(in srgb, var(--monad-gos-border) 80%, transparent);
		border-radius: 999px;
		background: color-mix(in srgb, var(--monad-gos-surface) 88%, transparent);
	...
	.nav-btn:hover {
		background: color-mix(in srgb, var(--monad-gos-accent-soft) 70%, transparent);
		color: var(--monad-gos-text);
	}

	.nav-btn.active {
		background: var(--monad-gos-surface);
		border-color: var(--monad-gos-border);
		color: var(--monad-gos-text);
	}
	...
	.composer-dock {
		...
		background: linear-gradient(to top, var(--monad-gos-bg) 75%, transparent);
	}
