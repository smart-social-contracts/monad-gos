/**
 * Monad MCP pairing-token API (Settings page).
 *
 ...
 */
import { createMcpPairing, isMockMode } from '../monad_gos_api.js';
...
export function getMcpBaseUrl() {
	const raw = import.meta.env.VITE_MONAD_MCP_URL || DEFAULT_MCP_BASE;
...
async function mockCreateToken({ label = '', scope, ttlDays }) {
	...
	const id = mockNextId++;
	const token = `mgosmcp_mock_${id}_${Date.now().toString(36)}`;
	const row = {
		id,
		label,
		scope,
