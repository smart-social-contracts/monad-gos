...
<section class="settings monad-gos-prose" aria-label="MCP settings">
	<header class="section-header">
		<h1>Settings</h1>
		<p class="monad-gos-muted intro">
			Connect Claude, ChatGPT, or another MCP client to <strong>Monad MCP</strong> — not
	...
	{:else}
		<div class="panel">
			...
			<p class="monad-gos-muted panel-hint">Use this URL in your MCP client configuration.</p>
			<div class="copy-row">
		...
		<div class="panel">
			...
			>
				...
				<label class="field">
					...
					<select class="monad-gos-input" bind:value={scope}>
						<option value="read">read — query Monad GOS</option>
				...
				<button type="submit" class="monad-gos-btn monad-gos-btn-primary" disabled={minting}>
		...
		{#if freshToken}
			<div class="panel fresh">
				...
				<p class="monad-gos-muted panel-hint">
		...
		<div class="panel">
			...
			{#if loading}
				<p class="monad-gos-muted">Loading tokens…</p>
			{:else if listError}
				<p class="error">{listError}</p>
			{:else if tokens.length === 0}
				<p class="monad-gos-muted">No pairing tokens yet.</p>
			{:else}
				<ul class="token-list">
					{#each tokens as row (row.id)}
						<li class="token-row">
							<div class="token-meta">
								...
								<span class="token-scope monad-gos-muted">{row.scope}</span>
								<span class="token-created monad-gos-muted">{formatDate(row.created_at)}</span>
							</div>
							<button
								type="button"
								class="monad-gos-btn monad-gos-btn-no"
		...
		{#if copyNotice}
			<p class="copy-notice monad-gos-muted" role="status">{copyNotice}</p>
		{/if}
	{/if}
</section>
