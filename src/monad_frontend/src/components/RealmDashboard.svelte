...
<section class="realm" aria-label="Realm dashboard">
	<header class="section-header">
		...
		<p class="monad-gos-muted intro">
			The live accounts of Monad GOS, under the Monad’s Codex.
		</p>
	</header>

	{#if !data}
		<p class="monad-gos-muted">Loading realm state…</p>
	{:else}
		...
		<p class="source">
			...
			{#if data.codex.commit}
				<span class="monad-gos-muted"> · {data.codex.commit.slice(0, 7)}</span>
			{/if}
		</p>
	{/if}
</section>
