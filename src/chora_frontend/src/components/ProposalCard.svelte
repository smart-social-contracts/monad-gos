...
<article class="proposal-card">
	<header>
		...
		<span class="status chora-muted">{proposal.status}</span>
	</header>
	...
	{#if proposal.thread_ids.length > 0}
		<div class="threads">
			<span class="chora-muted label">Motivated by</span>
			{#each proposal.thread_ids as threadId (threadId)}
				<button type="button" class="chora-btn chora-btn-ghost thread-btn" onclick={() => onopenthread?.(threadId)}>
					Read thread
				</button>
			{/each}
		</div>
	...
	{#if proposal.status === 'voting'}
		<div class="ballot" aria-label="Cast your vote">
			<button type="button" class="chora-btn chora-btn-yes" disabled={casting || voting} onclick={() => vote('yes')}>
				Yes
			</button>
			<button type="button" class="chora-btn chora-btn-no" disabled={casting || voting} onclick={() => vote('no')}>
				No
			</button>
			<button type="button" class="chora-btn" disabled={casting || voting} onclick={() => vote('abstain')}>
				Abstain
			</button>
		</div>
		{#if votePrompt}
